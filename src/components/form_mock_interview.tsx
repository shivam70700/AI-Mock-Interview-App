...
import CustomBreadCrumb from "./custom_breadcrumb";
...
const FormMockInterview = ({ initialData }: FormMockInterviewprops) => {
    ...
    const breadCrumpPage = initialData
        ? initialData.position
        : "Create";
    ...
    const cleanAiResponse = (data: string) => {
        ...
        // Extract a JSON array by capturing text between square brackets
        const jsonArrayMatch = result.match(/\[.*\]/s);

        if (jsonArrayMatch) {
            result = jsonArrayMatch[0];
        } else {
    ...
    return (
        <div className="w-full flex-col space-y-4">
            <CustomBreadCrumb breadCrumbPage={breadCrumpPage}
